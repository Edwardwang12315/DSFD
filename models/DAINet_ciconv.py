...
class DSFD(nn.Module):
	...
	def __init__(self, phase, base, extras, fem, head1, head2, num_classes):
		...
		# CIConv2d用于跨域特征提取
		self.coder = nn.Conv2d(1, 3, kernel_size=1, stride=1, padding=0)
	...
	def test_forward(self, x):
		...
		_x = x.clone()
		_x = self.ciconv2d(_x)
		detail = _x.clone()  # 保存细节图像
		
		_x = self.coder(_x)  # 通道扩展
		for k in range( 16 ) : 
			_x = self.vgg[ k ]( _x )
	...
	def forward(self, x):
		...
		return output,detail # 这里返回检测结果与CIConv2d的提取结果
...
def build_net_ciconv(phase, num_classes=2):
	base = vgg(vgg_cfg, 3)
